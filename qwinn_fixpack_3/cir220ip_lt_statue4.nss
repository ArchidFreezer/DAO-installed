...
void main()
{
    ...
    switch (nEventType)
    {
        ...
        case EVENT_TYPE_USE:
        {
            ...
            switch (nAction)
            {
                // Qwinn:  Had to convert this to an examinable placable to get vfx to work
                ...
                case PLACEABLE_ACTION_EXAMINE:
                {
                    ...
                    if(nStatue1 == TRUE && nStatue2 == TRUE && nStatue3 == TRUE && nStatue4 == FALSE)
                    {
                        WR_SetPlotFlag(PLT_LITE_TOW_REACH, TOW_REACH_STATUE_4, TRUE);

                        //Play success sound && ritual is now complete
                        ...
                        //AudioTriggerPlotEvent(35);
                        //add 7th codex entry for the plot
                        WR_SetPlotFlag(PLT_LITE_TOW_REACH, TOW_REACH_CREATURE_SPAWNED, TRUE);

                        //Can't click statues anymore
                        object oStatue1 = UT_GetNearestObjectByTag(oUser, CIR_IP_REA_STATUE1);
                        object oStatue2 = UT_GetNearestObjectByTag(oUser, CIR_IP_REA_STATUE2);
                        object oStatue3 = UT_GetNearestObjectByTag(oUser, CIR_IP_REA_STATUE3);
                        object oStatue4 = UT_GetNearestObjectByTag(oUser, CIR_IP_REA_STATUE4);
                        SetObjectInteractive(oStatue1, FALSE);
                        SetObjectInteractive(oStatue2, FALSE);
                        SetObjectInteractive(oStatue3, FALSE);
                        SetObjectInteractive(oStatue4, FALSE);
                        AudioTriggerPlotEvent(95);
                    }
                    else
                    {
                        //Damage player
