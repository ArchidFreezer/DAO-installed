...
{   
    ...
    if (GetHasEffects(oCreature, EFFECT_TYPE_INVALID, 300202)) 
    {
    ...
    if (nEffectType == EFFECT_TYPE_KNOCKDOWN || nEffectType == EFFECT_TYPE_STUN || nEffectType == EFFECT_TYPE_SLIP)
    {
        ...
        if ((nEffectType != EFFECT_TYPE_STUN) && (HasAbility(oCreature, 401200) == TRUE)) // GXA One With Nature
        {
            if (IsModalAbilityActive(oCreature, 401200) == TRUE)
            {
                return 3;
            }
        }
